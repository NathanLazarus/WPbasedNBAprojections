...
cap args argument1
cap global sims = `argument1' //getting sims from Stata 14 to 15
if _rc!=0{
	global sims = 3000000
}
...
twoway bar `1'a* teamid, ///
	...
	`labcolors' ytitle("`wrap_ytitle'", orientation(horizontal) size(small)) xtitle("") ylab(`6') ytick(`7', grid glcolor(gs11) glwidth(vvthin) glpattern(solid) nolab) ///
	plotregion(margin(zero)) color(`barcolors_b')  barw(`barwidth') title("${current_season} `2'") name(`sans_space', replace)
...
tograph wins "Projected Wins" millionodds.dta "Wins" nogroup 0(10)60 10(10)50
...
forvalues var = 7/11{
	...
	local transparency = cond(`var'==11,100,cond(`var'==10,85,cond(`var'==9,75,cond(`var'==8,65,cond(`var'==7,55,50)))))
	local intensity = 0.02+0.04*`var'+0.18*sqrt(`var'-2)
	...
	local reverse = `obs'-`i' +1
	local notorious_rgb=rgb[`i']
	local barcolors_a = `"`barcolors_a' "`notorious_rgb'%`transparency'*`intensity'""'
	local barwidth = `"`barwidth' 0.8"'
	}
	...
	local reverse_b = `obs'-`i' +1
	local notorious_rgb_b=rgb[`i']
	local barcolors_b = `"`barcolors_b' "`notorious_rgb_b'%`transparency'*`intensity'""'
	}
	...
	local marking = cond("``var''"=="champodds","Fin",cond("``var''"=="confodds","Cnf",cond("``var''"=="winsecond","2{sup:nd}",cond("``var''"=="winfirst","1{sup:st}",cond("``var''"=="make_playoffs","Pl","")))))
	gen markervar`var' = "`marking'" if ``var''>0.04

	local command`var' = `"bar ``var''a* teamid, color(`barcolors_a') barw(`barwidth') || bar ``var''b* teamid, color(`barcolors_b')  barw(`barwidth')|| scatter ``var'' teamid, ms(none) mlabpos(6) mlab(markervar`var') mlabcolor(black) mlabsize(*0.5) mlabgap(*0.01)"'
...
local sim_num = "`:di %12.0fc $sims'"
local sim_num = subinstr("`sim_num'"," ","",8)

twoway `command7' || `command8' || `command9' || `command10' || `command11' xlab(1(1)30, val ang(50) tl(0) labsize(*0.68)) ///
		`labcolors' ytitle("`wrap_ytitle'", orientation(horizontal) size(small)) xtitle("") ylab(`5') ytick(`6', grid glcolor(gs11) glwidth(vvthin) glpattern(solid) nolab) ///
		plotregion(margin(zero)) title("${current_season} `1'") subtitle("By Nathan Lazarus", color(white)) note("Based on `sim_num' simulations." " " "By Nathan Lazarus", size(vsmall) span) ///
		name(`sans_space', replace)
...
if "`needs_labels'"=="yes" {
	forvalues x = 1/`num_groups' {
		local groupname : label group_id `x'
		local location = `obs'*(`x'*2-1)/(`num_groups'*2)-0.35
		.`sans_space'.plotregion1.AddTextBox added_text editor -0.25 `location'
		//gr_edit .plotregion1.added_text_new = 1
		//gr_edit .plotregion1.added_text_rec = 1
		.`sans_space'.plotregion1.added_text[`x'].style.editstyle  angle(default) size(2.3) color(black) horizontal(center) vertical(middle) margin(zero) linegap(zero) drawbox(no) boxmargin(zero) fillcolor(bluishgray) linestyle( width(thin) color(black) pattern(solid)) box_alignment(0) editcopy
		.`sans_space'.plotregion1.added_text[`x'].text = {}
...
graph export `sans_space'fancy.png, replace

end

#delimit ;
fancygraph "Playoff Odds" playoff_odds.dta "Odds" conference `"0 "0" 0.25 "25" 0.5 "50" 0.75 "75" 1 "100""' 0.25(0.25)1
